...
WriteElementData (FILE * FP, DataType *Data) {
    ...
    for (n = Data->Element; n != NULL; n = g_list_next (n)) {
        ...
        if (!element->LineN && !element->PinN && !element->ArcN && !element->PolygonN
                && !element->PadN) {
            continue;
        }
        ...
        for (p = element->Polygon; p != NULL; p = g_list_next (p)) {
            PolygonType *polygon = p->data;
            int q, i = 0;
            Cardinal hole = 0;
            fprintf (FP, "\tElementPoly(%s)\n\t(", F2S (polygon, POLYGON_TYPE));

            for (q = 0; q < polygon->PointN; q++) {
                PointType *point = &polygon->Points[q];

                if (hole < polygon->HoleIndexN &&
                        q == polygon->HoleIndex[hole]) {
                    if (hole > 0) {
                        fputs ("\n\t\t)", FP);
                    }
