...
SmashBufferElement (BufferType *Buffer) {
    ...
    END_LOOP;
    POLYGON_LOOP (element);
    {
        poly2 = CreateNewPolygon(&Buffer->Data->SILKLAYER, polygon->Flags);
        CopyPolygonLowLevel(poly2, polygon);
    }
    END_LOOP;
...
#define MAYBE_WARN() \
    ...
    END_LOOP;
    POLYGON_LOOP (&Buffer->Data->SILKLAYER);
    {
        polygon2 = CreateNewPolygonInElement (Element, polygon->Flags);
        POLYGONPOINT_LOOP(polygon);
        {
            CreateNewPointInPolygon(polygon2, point->X, point->Y);
        }
        END_LOOP;
//         CopyPolygonLowLevel (polygon2, polygon);
        hasParts = true;
    }
    END_LOOP;
...
FreeRotateElementLowLevel (DataType *Data, ElementType *Element,
    ...
    POLYGON_LOOP (Element);
    {
        RotatePolygonLowLevel (polygon, X, Y, angle);
    }
    END_LOOP;
