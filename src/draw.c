...
set_object_color (AnyObjectType *obj, char *warn_color, char *selected_color,
    ...
    if      (warn_color      != NULL && TEST_FLAG (WARNFLAG,      obj)) {
        color = warn_color;
    } else if (selected_color  != NULL && TEST_FLAG (SELECTEDFLAG,  obj)) {
        color = selected_color;
    } else if (connected_color != NULL && TEST_FLAG (CONNECTEDFLAG, obj)) {
        color = connected_color;
    } else if (found_color     != NULL && TEST_FLAG (FOUNDFLAG,     obj)) {
        color = found_color;
    } else {
        color = normal_color;
    }
...
// Draw a little cross at each end of the line. BROKEN
/*
    gui->graphics->set_color (Output.fgGC, "magenta");
    gui->graphics->draw_line (Output.fgGC,
                              line->Point1.X - 2000, line->Point1.Y - 2000,
                              line->Point1.X + 2000, line->Point1.Y + 2000);
    gui->graphics->draw_line (Output.fgGC,
                              line->Point1.X + 2000, line->Point1.Y - 2000,
                              line->Point1.X - 2000, line->Point1.Y + 2000);
    gui->graphics->draw_line (Output.fgGC,
                              line->Point2.X - 2000, line->Point2.Y - 2000,
                              line->Point2.X + 2000, line->Point2.Y + 2000);
    gui->graphics->draw_line (Output.fgGC,
                              line->Point2.X + 2000, line->Point2.Y - 2000,
                              line->Point2.X - 2000, line->Point2.Y + 2000);
...
draw_element_package (ElementType *element) {
    ...
    ELEMENTPOLY_LOOP (element);
    {
        ...
        polygon->Clipped = PolygonToPoly(polygon);
        gui->graphics->fill_pcb_polygon (Output.fgGC, polygon, &(polygon->BoundingBox));
    }
    END_LOOP;
}
